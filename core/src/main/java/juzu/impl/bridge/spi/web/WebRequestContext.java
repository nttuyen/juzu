...
public abstract class WebRequestContext {
  ...
  public final void send(Response.Error error, boolean verbose) throws IOException {
    send(null, error.asStatus(verbose), true);
  }

  public final void send(AssetService assetPlugin, Response.Status response, boolean isFullPage) throws IOException {
    ...
    if (response instanceof Response.Content) {
      ...
      //
      stream = new WebStream((HttpStream)stream, assetManager, getRunMode().getMinifyAssets(), isFullPage) {
        @Override
